...
potlist = np.linspace(-0.1, -1.1, 1000)
...
data_to_save = np.hstack(([ZA_approx], H_approx))
np.savetxt('H_and_ZA.txt', data_to_save)
'''
...
data_loaded = np.loadtxt('H_and_ZA_big.txt')
ZA_approx = int(data_loaded[0])
H_approx = data_loaded[1:]
...
approx, H_frozen, ZA_frozen = BLDMC(
        ...
        N=100000000, M=200,
        q0=3, deltaq=0.01,
        potential=-1.10, lam=0,
        H_approx=H_approx, ZA_approx=ZA_approx
    )

data_to_save2 = np.hstack(([ZA_frozen], H_frozen))
np.savetxt('H_and_ZA_big.txt', data_to_save2)


#plt.plot(potlist, num,color = 'red', label='Numerical')
plt.scatter(-1.10, approx,color = 'red', label='Numerical')
