...
def R12(A12,A21,q0,q1,q,chi,deltaq,H,potential,r_star,mass,eps = epsilon):
    acceptance_ratio = ( (A21) / (A12) ) * ( (2*q0*abs(u(np.sqrt((q**2) + (q1**2) - 2*q*q1*chi)) * H[get_bin(q1,deltaq)])) / (np.pi * abs(u(q,potential,r_star,mass,eps = eps))) )
    return acceptance_ratio

def Type1to2(r,A12,A21,q0,q1,q,chi,deltaq,H,potential,r_star,mass,eps = epsilon):
    '''
    ...
    assert 0 <= r <= 1
    if r <= A12:
        if np.random.random() <= Pxy(R12(A12,A21,q0,q1,q,chi,deltaq,H,potential,r_star,mass,eps = eps)):
            return True
    return False
...
def variables1(r,A10,A12,lam,q,potential,r_star,mass,eps = epsilon):
    assert 0 <= r <= 1
    if r <= (1 - A10 - A12):
...
def R21(A12,A21,q0,q1,q,chi,deltaq,H,potential,r_star,mass,eps = epsilon):
    acceptance_ratio = heaviside(q0,q1) * ( (A12) / (A21) ) * ( (np.pi * abs(u(q,potential,r_star,mass,eps = eps))) / (2*q0*abs(u(np.sqrt((q**2) + (q1**2) - 2*q*q1*chi),potential,r_star,mass,eps = eps) * H[get_bin(q1,deltaq)])) )
    return acceptance_ratio

def Type2to1(r,A12,A21,q0,q1,q,chi,deltaq,H,potential,r_star,mass,eps = epsilon):
    '''
    ...
    assert 0 <= r <= 1
    if r <= A21:
        if np.random.random() <= Pxy(R21(A12,A21,q0,q1,q,chi,deltaq,H,potential,r_star,mass,eps = eps)):
            return True
    return False

# ================================ TYPE 2 ---> VARIABLE ================================

def R2(q,q1,qp,qp1,chi,deltaq,H,potential,r_star,mass,eps = epsilon):
    '''
    qp = q prime
    qp1 = q prime 1
    '''
    acceptance_ratio = (abs(u(np.sqrt(qp**2 + qp1**2 - 2*qp*qp1*chi),potential,r_star,mass,eps = eps) * H[get_bin(qp1,deltaq)]) * q*q1) / (abs(u(np.sqrt(q**2 + q1**2 - 2*q*q1*chi),potential,r_star,mass,eps = eps) * H[get_bin(q1,deltaq)]) * qp*qp1)
    return acceptance_ratio

def variables2(r,q0,A21,lam,q,q1,chi,deltaq,H,potential,r_star,mass,eps = epsilon):
    assert 0 <= r <= 1
    if r <= (1 - A21):
        chi = np.random.uniform(-1, 1)
        qp = lambda_seed(lam,q)
        qp1 = lambda_seed(lam,q1)
        if np.random.random() <= Pxy(R2(q,q1,qp,qp1,chi,deltaq,H,potential,r_star,mass,eps = eps)):
...
def DiagMC(N,A10,A12,A21,q0,D0,deltaq,lam,potential,r_star,mass,eps = epsilon):
    ...
    for _ in range(N):
        ...
        if Type == 1:
            ...
            elif Type1to2(r,A12,A21,q0,q1,q,chi,deltaq,H,potential,r_star,mass,eps = eps):
                Type = 2
                type1sum += 1
                continue

            else:
                q = variables1(r,A10,A12,lam,q,potential,r_star,mass,eps = eps)
                H = U(1,q,H,deltaq)
                type1sum += 1
                continue
        ...
        if Type == 2:
            if Type2to1(r,A12,A21,q0,q1,q,chi,deltaq,H,potential,r_star,mass,eps = eps):
                Type = 1
                type2sum += 1
                continue

            else:
                q,q1,chi = variables2(r,q0,A21,lam,q,q1,chi,deltaq,H,potential,r_star,mass,eps = eps)
                H = U(2,q,H,deltaq)
                type2sum += 1
                continue
