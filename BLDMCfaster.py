...
@njit
def RAB(q,qprime,PAB,lam,qvals,deltaq,ZA,H,potential,chi,I_u):

    numerator1 = 2 * abs(1 + lam) * If(qvals,deltaq,ZA,H,potential,I_u)
...
@njit
def RBA(q,qprime,PAB,lam,qvals,deltaq,ZA,H,potential,chi,I_u):
    return 1/RAB(q,qprime,PAB,lam,qvals,deltaq,ZA,H,potential,chi,I_u)

# ============================ A -> C, C -> A ============================

@njit
def RAC(q,qprime,PCA,PAC,lam,qvals,deltaq,potential,chi,I_u):
    numerator1 = 2 * abs(lam) * I_u * PCA
...
def RCA(q,qprime,PCA,PAC,lam,qvals,deltaq,potential,chi,I_u):
    return 1/RAC(q,qprime,PCA,PAC,lam,qvals,deltaq,potential,chi,I_u)

# ============================ C -> D, D -> C ============================

@njit
def RCD(qprime,qdprime,PCD,qvals,deltaq,ZA,H,potential,chi,I_u):
    '''
    qdprime := q double prime
    '''
    numerator1 = 2 * If(qvals,deltaq,ZA,H,potential,I_u)
...
@njit
def RDC(qprime,qdprime,PCD,qvals,deltaq,ZA,H,potential,chi,I_u):
    return 1/RCD(qprime,qdprime,PCD,qvals,deltaq,ZA,H,potential,chi,I_u)

# ============================ HISTOGRAM UPDATER ============================

@njit
def HistogramBuilder(Type,q,qprime,qdprime,chi,potential,ZA_frozen,qvals,deltaq,H_frozen,H_measured,I_u,chip):
    ...
    if Type == 0:
        ...
        sign = -np.sign(u(usqrt(q,qprime,chi),potential) * Fq(qprime,ZA_frozen,qvals,deltaq,H_frozen,potential,I_u))  # MAYBE SWAP B AND C
    elif Type == 2:
        sign = -np.sign(u(usqrt(q,qprime,chi),potential) * u(qprime,potential))
    elif Type == 3:
        sign = -np.sign(u(usqrt(q,qprime,chi),potential) * u(usqrt(qprime,qdprime,chip),potential) * Fq(qdprime,ZA_frozen,qvals,deltaq,H_frozen,potential,I_u))
...
def BLDMC(PAA,PAB,PAC,PCA,PCD,PBA,PDC,N,M,q0,deltaq,potential,lam): #PBA = PDC = 1
    ...
    for i in range(M):
        H_measured[:] = 0.0
        DiagramAsum = DiagramBsum = DiagramCsum = DiagramDsum = 0
        ...
        for _ in range(N):
            ...
            if Type == 0:
                if r < PAA:
                    ...
                    r2 = np.random.random()
                    if r2 < PXY(RAB(q,qprime,PAB,lam,qvals,deltaq,ZA_frozen,H_frozen,potential,chi,I_u)):
                        Type = 1
                ...
                if r < PBA:
                    r2 = np.random.random()
                    if r2 < PXY(RBA(q,qprime,PAB,lam,qvals,deltaq,ZA_frozen,H_frozen,potential,chi,I_u)):
                        Type = 0
                ...
                if r < PCA:
                    ...
                    r2 = np.random.random()
                    if r2 < PXY(RCD(qprime,qdprime,PCD,qvals,deltaq,ZA_frozen,H_frozen,potential,chip,I_u)):
                        Type = 3
            
            elif Type == 3:
                if r < PDC:
                    r2 = np.random.random()
                    if r2 < PXY(RDC(qprime,qdprime,PCD,qvals,deltaq,ZA_frozen,H_frozen,potential,chip,I_u)):
                        Type = 2
            
            if Type == 0:
                H_measured = HistogramBuilder(0,q,qprime,qdprime,chi,potential,ZA_frozen,qvals,deltaq,H_frozen,H_measured,I_u,chip)
                DiagramAsum += 1
            elif Type == 1:
                H_measured = HistogramBuilder(1,q,qprime,qdprime,chi,potential,ZA_frozen,qvals,deltaq,H_frozen,H_measured,I_u,chip)
                DiagramBsum += 1
            elif Type == 2:
                H_measured = HistogramBuilder(2,q,qprime,qdprime,chi,potential,ZA_frozen,qvals,deltaq,H_frozen,H_measured,I_u,chip)
                DiagramCsum += 1
            elif Type == 3:
                H_measured = HistogramBuilder(3,q,qprime,qdprime,chi,potential,ZA_frozen,qvals,deltaq,H_frozen,H_measured,I_u,chip)
                DiagramDsum += 1
        ...
        print(f'Iteration: {i}, Approximation: {approx}')
...
potlist = np.linspace(-0.1, -1.5, 30)
potlistann = np.linspace(-0.1,-1.5,1000)
num = []
ann = []


for i in potlist:
    num.append(BLDMC(PAA=0.2,PAB=0.4,PAC=0.4,PCA=0.5,PCD=0.5,PBA=1,PDC=1,N = 1000000,M = 3,q0=10,deltaq=0.01,potential=i,lam=1))
...
plt.plot(potlist,num,label = 'Numerical')
plt.plot(potlistann,ann,label = 'Analytical')
plt.ylim(-10,10)
plt.legend()
plt.show()
